...
WORKDIR /app
ENV DOTNET_USE_POLLING_FILE_WATCHER=1 ASPNETCORE_URLS=http://0.0.0.0:8055
EXPOSE 8055
...
| `EXPOSE 8055` | **Documenta** en qué puerto escucha el programa | No abre nada: quien publica el puerto es el `ports:` del compose |
...
flowchart LR
    ...
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:15455"| POSTGRES
    CLIENTE -.->|"opcional (diagnóstico):<br/>localhost:11455"| SQLSERVER
```
...
docker network create proyecto_diseno_de_software4_default

# 2. postgres
docker run -d --name postgres --network proyecto_diseno_de_software4_default --restart unless-stopped `
  ...
  -p 15455:5432 postgres:16-alpine

# 3. ESPERAR a que responda de verdad… mirándolo a ojo

# 4. sqlserver
docker run -d --name sqlserver --network proyecto_diseno_de_software4_default --restart unless-stopped `
  -e "ACCEPT_EULA=Y" `
  -e "MSSQL_SA_PASSWORD=Diseno123!" `
  -e "MSSQL_PID=Developer" `
  -v mssqldata:/var/opt/mssql `
  -p 11455:1433 mcr.microsoft.com/mssql/server:2022-latest

# 5. ESPERAR a que responda de verdad… mirándolo a ojo

# 6. sqlserver-init
docker run -d --name sqlserver-init --network proyecto_diseno_de_software4_default --restart no --entrypoint /bin/bash `
  -e "MSSQL_SA_PASSWORD=Diseno123!" `
...
docker run -d --name api-facturas --network proyecto_diseno_de_software4_default --restart unless-stopped `
...
```yaml
URL_API: http://api-facturas:8055
```
...
|---|---|---|
| **Su navegador** | `http://localhost:8055` | Está fuera de Docker. Usa el puerto **publicado** |
| **Otro contenedor** | `http://api-facturas:8055` | Están en la misma red: se llaman por el **nombre del servicio** |
...
> **En este proyecto los dos números son iguales** (`8055:8055`), y eso
...
Si alguien cambiara esa línea por `http://localhost:8055`, el front
...
```powershell
docker network inspect proyecto_diseno_de_software4_default
```
...
`docker-compose.yml` y le pega `_default`. La carpeta `proyecto_diseno_de_software4` produce
la red `proyecto_diseno_de_software4_default`. Tres formas de confirmarlo:
...
docker inspect proyecto_diseno_de_software4-api-facturas-1 --format '{{range $k,$v := .NetworkSettings.Networks}}{{$k}}{{end}}'
```
...
network proyecto_diseno_de_software4_default has active endpoints (name:"proyecto_diseno_de_software4-api-facturas-1" …)
```
